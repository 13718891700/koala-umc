...
public class SecurityRealm extends AuthorizingRealm{
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException{
	   ...
	   /*CmsUser user = null;
		try {
            user = cscUserService.getUserByUserName(token.getUsername());
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(user == null){
		...
		SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(user.getUserName(),user.getLoginPwd(),ByteSource.Util.bytes(user.getSalt()),getName());
