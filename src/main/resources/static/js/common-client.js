...
CommonClient.syncpost = function(url,param,successFunction){
	CommonClient.ajax("POST", CommonClient.parseUrl(url), param, successFunction,false);
};
...
CommonClient.post = function(url,param,successFunction){
	CommonClient.ajax("POST", CommonClient.parseUrl(url), param, successFunction,true);
};
...
CommonClient.get = function(url,param,successFunction, isUnMusk){
	CommonClient.ajax("GET", CommonClient.parseUrl(url), param, successFunction,true,isUnMusk);
};

CommonClient.getNoBeforeEnd = function(url,param,successFunction){
	CommonClient.ajaxNoBeforeEnd("GET", CommonClient.parseUrl(url), param, successFunction);
};

var urlMapping;
CommonClient.parseUrl= function(url){
	$.ajax({
		url : "/uam/url/getUrlMapping",
		type : 'POST',
		dataType : "json",
		async : false,
		cache : false,
	...
	var mp  = urlMapping.result;
	// console.info(mp);
	for(var i in mp){
		var bool = url.indexOf(i);
		if(bool===0){
			return mp[i] + url;
		}
	}
	return url;
	// urlMapping=[{name:"/csc/",url:"http://localhost:7007"},{name:"/ofc/",url:"http://localhost:8080"}]
